...
export const SurveyList = ({ navigation }) => {
  ...
  const {
    ...
    urlPath: API_PATH.SECURE.SURVEY.FILTER({ search: form.search }),
    reactQueryOptions: {
      enabled: true,
      onError: (err) => {
      ...
      <LinearGradient colors={["#02367B", "#72A5D3"]} style={[styles.container, { paddingTop: insets.top + 38 }]}>
        ...
        {isLoadingSurveyList ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => <SurveyItem item={item} navigation={navigation} />}
          />
        )}
      </LinearGradient>
    </>
